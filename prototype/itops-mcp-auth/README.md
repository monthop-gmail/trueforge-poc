...
17 เคสผ่าน ครอบ static ทั้งสาม role, ตัวตนที่ map แล้วและยังไม่ map, หมดอายุ, ผิด issuer,
ผิด audience, ไม่รู้จัก kid, payload ถูกแก้ · และยืนยันว่า JWKS ถูกดึง **1 ครั้ง** ตลอด 17 เคส
...
1. **ฮับยังไม่อ่าน `X-Actor`** — Nginx ส่งให้แล้วในระดับ config แต่ `mcp-hub` ยังไม่เอาไปลง log
   ของตัวเอง ยังไม่มีหลักฐานฝั่งปลายทาง เป็นงานชิ้นถัดไปที่เล็กที่สุดและได้ audit รายคนเต็มรูป
2. **ยังไม่เคยเจอ JWT จริงของ gateway** — ทดสอบด้วยคีย์ทดสอบในเครื่อง ตัว shape ของ claims
   (`email` / `sub` / `aud`) ต้องยืนยันกับโทเคนจริงก่อน แล้วอาจต้องปรับ mapping
3. **request ที่ถูกปฏิเสธ ยังไม่มีตัวตนใน access log** (`auth_request` ไม่คืนค่าเมื่อ deny) —
   ตัวตนของความพยายามที่ล้มเหลวอยู่ใน log ของ `mcp-auth` แทน ถ้าต้องการรวมที่เดียวต้องทำเพิ่ม
4. **ยังไม่มี test เรื่อง JWKS ล่ม / rotate คีย์กลางคัน** ในชุด smoke
5. **การ map principal → role** ตอนนี้เป็น JSON ใน env เหมาะกับคนไม่กี่คน ถ้าจะโตต้องมีที่เก็บจริง
