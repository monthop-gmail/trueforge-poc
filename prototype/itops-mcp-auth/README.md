...
- tool ที่ถูกปฏิเสธถูกลงทะเบียนแล้ว **`disable()`** ทันที ได้ทั้งสองชั้นพร้อมกันตามที่ทีมขอใน
  seq 20: SDK กรอง tool ที่ disabled ออกจาก `tools/list` (ลดโอกาสที่ agent จะพยายามเรียก) และ
  ยังตอบ `Tool <name> disabled` เมื่อเรียกตรง ๆ ซึ่งเป็นการปฏิเสธที่ระบุชื่อ ไม่ใช่ `unknown tool`
  ที่แยกไม่ออกจากการพิมพ์ผิด
- รอบแรกใช้ refusal handler ของเราเองพร้อมข้อความที่บอกเหตุผล แล้วพบว่า SDK ตรวจ argument
  ก่อนเรียก handler ผู้เรียกที่ไม่ส่งฟิลด์บังคับจึงได้ "argument ผิด" แทนคำปฏิเสธ · พอเปลี่ยนมาใช้
  `disable()` ปัญหานั้นหายไปด้วย เพราะ SDK ปฏิเสธก่อนถึงชั้น validation
- นโยบายของเส้นทางถูกบันทึกลง log ตอน boot ส่วนการบันทึกรายครั้งว่าใครเรียก tool ที่ถูกปฏิเสธ
  ต้องมี hook ที่ชั้น HTTP ซึ่งอยู่ใน `mcp-common` ที่ทุกบริการใช้ร่วมกัน จึงไม่แตะในรอบนี้
